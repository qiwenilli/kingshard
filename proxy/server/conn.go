...
func (c *ClientConn) readHandshakeResponse() error {
    ...
    //check 用户名
    customeUser, err := config.GetUser(c.proxy.cfg.Userconfig_path,c.user)
    if err !=nil{
        return mysql.NewDefaultError(mysql.ER_PASSWORD_NO_MATCH, c.user, "用户不存在", "No")
    }
	...
	//}
    checkAuth := mysql.CalcPassword(c.salt, []byte(customeUser.Pwd))
	if c.user != customeUser.User || !bytes.Equal(auth, checkAuth) {
		golog.Error("ClientConn", "readHandshakeResponse", "error", 0,
			"auth", auth,
			"checkAuth", checkAuth,
			"client_user", c.user,
			"config_set_user", customeUser.User,
			"passworld", customeUser.Pwd)
		return mysql.NewDefaultError(mysql.ER_ACCESS_DENIED_ERROR, c.user, c.c.RemoteAddr().String(), "Yes")
	}
